...
def validate_request(request: ChatCompletionRequest):
    ...
    if model_info.is_image_edit:
        ...
        if not user_messages:
            raise HTTPException(status_code=400, detail="image edit requires a user message with image content")
        ...
        for block in last_user.content:
            ...
            if block.get("type") == "text" and block.get("text"):
                ...
                image_url = image_payload.get("url") if isinstance(image_payload, dict) else None
                if not image_url:
                    continue
                ...
                if image_url.startswith("data:") and "," in image_url:
                    ...
                    images.append(UploadFile(file=tmp, filename=f"image-{len(images)+1}.{ext}", headers={"content-type": mime}))
                else:
                    import httpx
                    ...
                    filename = parsed.path.rsplit('/',1)[-1] or f"image-{len(images)+1}"
                    mime = response.headers.get("content-type", "application/octet-stream")
                    images.append(UploadFile(file=tmp, filename=filename, headers={"content-type": mime}))

        prompt = "
".join(p.strip() for p in prompt_parts if p and p.strip())
        if not prompt:
            raise HTTPException(status_code=400, detail="image edit requires a text prompt")
        if not images:
            raise HTTPException(status_code=400, detail="image edit requires at least one image")
        ...
        edit_result = await grok_service.edit_image(edit_request)
        if not edit_result.data or not isinstance(edit_result.data, list):
            raise HTTPException(status_code=500, detail="image edit returned no image data")

        image_markdown = f"![image]({edit_result.data[0]})"
        return {
            ...
            "choices": [{
                "index": 0,
                "message": {"role": "assistant", "content": image_markdown},
                "finish_reason": "stop"
            }]
        }
    # image 验证
